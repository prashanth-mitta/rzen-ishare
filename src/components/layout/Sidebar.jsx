...
import { LayoutDashboard, Building2, GitBranch, Users, Clock, MapPin, Briefcase, FolderKanban, FileText, LogOut, ShieldCheck, X } from 'lucide-react'
...
export default function Sidebar({ open, onClose }) {
  ...
  return (
    <>
      ...
      <aside className={`
        fixed lg:sticky top-0 left-0 h-screen w-64 lg:w-60 shrink-0 z-50
        flex flex-col border-r border-gray-200 bg-white
        transform transition-transform duration-200 ease-in-out
        ${open ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0
      `}>
        <div className="flex items-center justify-between gap-2.5 px-4 py-4 border-b border-gray-100">
        ...
        <div className="px-3 py-3 border-t border-gray-100">
          <div className="flex items-center gap-2.5 px-2 py-2 rounded-lg">
            <div className="w-8 h-8 rounded-full bg-brand-100 flex items-center justify-center text-xs font-semibold text-brand-700 shrink-0">{initials}</div>
            ...
            <button onClick={handleSignOut} className="text-gray-400 hover:text-red-500 transition-colors shrink-0" title="Sign out"><LogOut size={15}/></button>
          </div>
        </div>
      </aside>
    </>
